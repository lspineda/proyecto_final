		...
		<div class="text-center col-md-12" v-if="pageData.last_page > 1">
			<ul class="pagination">
				<li :class="[((pageData.current_page == 1) ? 'disabled' : '')]">
					<a :href="'?page=' + pageData.current_page" @click.prevent="pageClicked(pageData.current_page - 1)"
						aria-label="Previous" v-if="pageData.current_page != 1">
				...
				<li :class="[((pageData.current_page == pageData.last_page) ? 'disabled' : '')]">
					<a :href="'?page=' + pageData.current_page" @click.prevent="pageClicked(pageData.current_page + 1)"
						aria-label="Next" v-if="pageData.current_page != pageData.last_page">
						<span aria-hidden="true">»</span>
					</a>
					<a v-else>
						<span aria-hidden="true">»</span>
