			...
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					...
					<div class="modal-header">
						<h4 class="modal-title" id="CreatePayment">Pagos de factura N° : {{ id }} <br> Cliente : {{
							invoice.customer.name }} </h4>
					</div>
							...
							<table class="table table-bordered table-condensed">
								...
								<tbody>
									<tr v-for="payment in payments">
										<td>{{ payment.payment_date | moment('LL') }}</td>
										<td>{{ payment.amount }}</td>
										<td>{{ payment.paid_in }}</td>
										<td>{{ payment.bank_information }}</td>
										<td>{{ payment.user.name }}</td>
										<td class="align-center">
											<button @click="deletePayment(payment.id)" type="button"
					...
					<div class="modal-footer">
						<button @click="closeModal" type="button" class="btn btn-danger waves-effect"
